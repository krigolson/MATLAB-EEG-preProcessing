function EEG = doOcularCorrection(EEG, selection_cards, varargin)
    
% gratton_emcp() - Run the eye movement correction procedure developed
%                  by Gratton, Coles, and Donchin (1983) on an epoched eeglab
%                  dataset.
% this version was developed by Bill Gehring and modified by Kyle Mathewson
% and Olave Krigolson
%
% NOTE! This technique is most effective when it is run on data with ocular
% electrodes, see below for usage. For demonstration purposes only we are
% using a data set and one electrode above the eye
%
% Usage:
%   >> EEG = gratton_emcp( EEG, selection_cards, eog_channel_pairs )
%
% Correct for a single EOG channel (must be a bipolar vertical channel, corrects only blinks and vertical EOG)
%
